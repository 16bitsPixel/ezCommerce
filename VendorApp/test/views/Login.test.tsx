...
it('renders login form', () => {
  global.fetch = jest.fn(() =>
    Promise.resolve({
      json: () => Promise.resolve({ data: { login: { name: 'Test User', accessToken: 'test-token', role: 'vendor' }, isVerified: true } }),
    })
  ) as jest.Mock;
  render(
    <Login />
...
it('renders signup form - for context', () => {
  render(
    <Login />
  );
...
it('clicks signup button', async () => {
  ...
  fireEvent.click(screen.getByText("Don't have an account? Sign Up"))
  await waitFor(() => {
    expect(mocksetView).toHaveBeenCalledWith('Signup');
  });
});
  ...
  global.fetch = jest.fn()
    .mockResolvedValueOnce({
      json: () => Promise.resolve({
        data: { login: { name: 'Test User', accessToken: 'test-token', role: 'vendor' } }
      })
    })
    .mockResolvedValueOnce({
      json: () => Promise.resolve({
